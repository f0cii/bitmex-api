...
// SetProxyURL set connect proxy url
func (rc *RecConn) SetProxyURL(proxyURL string) {
...
func (rc *RecConn) setDefaultDialer(handshakeTimeout time.Duration) {
	...
	if rc.proxyURL != "" {
		proxyURL_, err := url.Parse(rc.proxyURL)
		if err != nil {
			return
		}
		rc.dialer.Proxy = http.ProxyURL(proxyURL_)
	}
}
